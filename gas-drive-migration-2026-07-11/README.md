...
セットアップは **mise + pnpm + clasp**。`clasp create-script` で「スプレッドシート+
バインドプロジェクト」を一度に作り、以後の操作はシートのメニューだけで完結する
(詳細は [第3章](./docs/textbook/03_setup_guide.md))。

```bash
# 前提: 移行先の共有ドライブに移行元アカウントが「コンテンツ管理者」で追加済み
cd gas-drive-migration-2026-07-11
mise trust && mise install          # Node.js + pnpm を導入
mise run setup                      # pnpm install
mise run login                      # clasp で移行元アカウントにログイン
                                    # (事前に script.google.com/home/usersettings で Apps Script API を ON)
mise run create-sheet               # 新規シート+バインドプロジェクトを作成 (URL が表示される)
mise run push                       # dist/ を push
```

その後、作成されたスプレッドシートを開いて:

1. メニュー **「📁 ドライブ移行」→「① 設定シートを準備」**(初回は権限承認)
2. 「設定」シートに **移行元/移行先フォルダ ID** と各設定を入力 (黄色い列のみ)
3. メニュー **「② 移行を開始」**。まず「ドライラン」ONで計画確認 → OFFで本実行
   (進捗は「進捗」シートに自動表示、または「③ 進捗を更新して表示」)

> 開発環境を用意できない場合は、[`dist/main.js`](./dist/main.js) をシートの
> 「拡張機能 → Apps Script」に貼り付けるだけでも動く (詳細は第3章 3.9 の補足)。

## 🛠 開発コマンド (コードや図を修正する人向け)

```bash
mise run build               # dist/ へビルド (tsc)
mise run typecheck           # 型チェックのみ
mise run push                # ビルドして GAS プロジェクトへ反映
...
├── mise.toml               # ツール・タスク定義 (node / pnpm)
├── pnpm-workspace.yaml     # pnpm 設定 (minimumReleaseAge)
