...
function startMigration() {
    ...
    if (existing && (existing.status === 'RUNNING' || existing.status === 'SUSPENDED')) {
        throw new Error('進行中の移行ジョブがあります。printStatus で状況を確認し、' +
    ...
    clearStateStorage_();
    // --- 移行元の検証 ---
    const src = withRetry_('移行元フォルダの取得', () => Drive.Files.get(CONFIG.SOURCE_FOLDER_ID, {
    ...
    if (src.driveId) {
        Logger.log('⚠ 移行元が既に共有ドライブ内にあります。このスクリプトはマイドライブ→共有ドライブを想定しています。' +
    ...
    const rootDst = CONFIG.CREATE_TOP_FOLDER
        ...
        : dest.rootId;
    state.queue.push({ src: src.id, dst: rootDst, path: srcName });
    saveState_(state);
    runLoop_(state);
}
/**
...
function trashEmptySourceFolders() {
    ...
    if (!state || state.status !== 'DONE') {
        Logger.log('⚠ 移行が DONE になっていません。printStatus で完了を確認してから実行することを推奨します。' +
...
function resolveDestination_(destId) {
    ...
    if (!folder.driveId) {
        throw new Error(`DEST_FOLDER_ID (${folder.name}) は共有ドライブ内にありません。` +
...
function ensureFolder_(name, dstParentId, state, pathForLog) {
    ...
    const created = withRetry_('移行先フォルダの作成', () => Drive.Files.create({ name: name, mimeType: FOLDER_MIME, parents: [dstParentId] }, null, { supportsAllDrives: true, fields: 'id' }));
    state.stats.foldersCreated += 1;
...
function suspendAndScheduleResume_(state) {
    ...
    saveState_(state);
    scheduleResume_();
    Logger.log(`⏸ 制限時間に達したため中断しました (残りフォルダ: ${state.queue.length})。` +
...
function finishMigration_(state) {
    ...
    Logger.log(report);
    notify_(state.dryRun
        ? '【DRY_RUN 完了】ドライブ移行の計画確認が終わりました'
        : '【完了】ドライブ移行が終わりました', report);
}
// =====================================================================
...
function saveState_(state) {
    ...
    if (json.length > STATE_TOTAL_LIMIT) {
        throw new Error(`移行状態が大きくなりすぎました (${json.length} 文字)。` +
