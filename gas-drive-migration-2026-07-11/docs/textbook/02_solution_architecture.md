...
[← 第1章](./01_background.md) | [目次](./README.md) | [次章: セットアップ →](./03_setup_guide.md)
...
なお「リッチな操作画面が無い」という GAS の弱点は、本ツールでは
**スプレッドシートを操作画面に仕立てる**ことで補っている (次項)。拡張機能なしでも、
利用者はコードを一切触らず、スプレッドシートのメニューと設定シートだけで
移行を実行できる。

### 操作インターフェースは「スプレッドシート」

本ツールは Google スプレッドシートに紐づく **container-bound スクリプト**として作り、
スプレッドシートを唯一の操作画面にしている。

- **「設定」シート**: 移行元/移行先フォルダ ID や各種設定を入力 (コード編集は不要)
- **カスタムメニュー「📁 ドライブ移行」**: 開始・再開・中止・進捗確認・後片付けを実行
- **「進捗」/「失敗一覧」シート**: 実行状況と失敗明細をリアルタイム表示

![スプレッドシート UI](./plantuml/out/09_spreadsheet_ui.svg)

これにより、GAS エディタを開いて `CONFIG` を書き換える…といった開発者向けの操作は
不要になり、スプレッドシートに慣れた利用者なら誰でも移行を実行できる。

<details>
<summary>📖 用語解説: コンテナバインド (container-bound) スクリプト</summary>

スプレッドシートなどの Google ファイルに紐づけて作る Apps Script。
`拡張機能 → Apps Script` から作成するとこの形式になり、紐づいたシートを直接
操作したり、シートを開いた瞬間にメニューを足す `onOpen` が使えたりする。
これが「スプレッドシートを操作画面にする」ための土台になっている。
...
| 移行先 (`@misugi-corp.co.jp`) | 移行元ユーザーのファイルはすべて「他人所有」に見えるため、本ツールでは**全ファイルがスキップされ何も移行されない**(他人所有ファイルは移動もコピーもしない設計 → [第6章 6.6](./06_operations.md#66-他人所有ファイルはスキップされる)) | ❌ |
...
> マウスでレイアウトを調整したい場合はそちらを編集する (→ [第5章](./05_dev_environment.md))。
