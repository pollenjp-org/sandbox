...
| 移動元に案内ファイル(.txt)を残す | | 各フォルダの中身を移動後、移動元に「このフォルダの中身は移動しました.txt」を作り、移行先リンクと**移動した全ファイルの URL** を記載。既定ON (→ [3.16](#316-移動元に残る案内ファイル)) |
...
**フォルダの URL は必ず変わる**(移動ではなく作り直しのため → [6.2](./06_operations.md#62-移行で維持されるもの変わるもの))。
そこで移行先フォルダのリンクだけでなく、**そのフォルダにあった全ファイルの URL**
を 1 行ずつ記載する。移動したファイルは ID が変わらないので、この一覧があれば
旧フォルダを開いた人が目的のファイルへ直接辿り着ける。

各行の `[...]` はそのファイルがどうなったかを表す:

| 表示 | 意味 | URL |
| --- | --- | --- |
| `[移動]` | 移行先へ移動した | 移行前と**同じ** |
| `[コピー]` | 移動できずコピーで救済した | **変わる**。旧 URL を併記する |
| `[未移動(他人所有)]` | 他人所有のためスキップし、このフォルダに残っている (→ [3.17](#317-他人所有ファイルはスキップされる)) | 変わらない |
| `[未移動(失敗)]` | 移動もコピーも失敗し、このフォルダに残っている | 変わらない |

- この案内ファイル自身は**移行先へは移動されない**(移動元に残る目印のため)。
  再実行しても移動されず、内容だけが最新のリンク・日時に更新される(冪等)
- 再実行や自動中断・再開では「残っているファイルだけ」が処理されるため、既存の
  案内ファイルの一覧を**読み戻してマージ**する。前回移動したファイルの行が
  消えることはない
- 1 フォルダあたりの記載は 5000 件まで。超えた分は件数だけを記し、全件は
  スプレッドシートと実行ログで確認する
- ファイルが残ったフォルダでは見出しが「一部を移動しました(N 件はこのフォルダに
  残っています)」に変わる。**中身が残っているのに「全部移動した」とは書かない**
- ドライラン中は作成されず、作成予定だけがログに出る
- 「【後片付け】空フォルダを削除」を実行すると、案内ファイルだけが残るフォルダは
  「空」とみなされ、案内ファイルごとゴミ箱に入る。**一覧を残したい場合は後片付けを
  実行しないか、事前に案内ファイルを控えておくこと**
