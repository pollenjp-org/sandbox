...
interface SettingDef {
  /** コード内部で使うキー */
  key: string;
  /** 設定シートの A 列に表示するラベル (これをキーに値を読む) */
  label: string;
  /** C 列に表示する説明 */
  description: string;
  type: SettingType;
  default: string | number | boolean;
  /** true の場合、空だと実行前にエラーにする */
  required?: boolean;
}
...
const SETTING_DEFS: SettingDef[] = [
  {
    ...
    label: '移行元フォルダID',
    description:
      'マイドライブで対象フォルダを開いた URL の /folders/ 以降の文字列。例: 1AbCdEf...',
    type: 'string',
    default: '',
    required: true,
  },
  {
    ...
    description:
      '共有ドライブそのもの、または共有ドライブ内フォルダの ID。URL の /folders/ 以降。マイドライブ内は指定不可。',
    type: 'string',
    default: '',
    required: true,
  },
  {
    ...
    description:
      'チェックすると一切変更せず、実行予定の操作を「進捗」シートとログに出すだけ。まずこれで確認し、本実行時にチェックを外す。',
    type: 'boolean',
    default: true,
  },
  {
    ...
    description:
      'チェック: 移行先に「移行元と同名フォルダ」を作りその中へ移行。外す: 移行先フォルダ直下へ中身を直接展開。',
    type: 'boolean',
    default: true,
  },
  {
    ...
    label: '移動失敗時にコピーで救済',
    description:
      '自分が所有するファイルの移動が失敗した場合にコピーで救済する。コピーはファイル ID が変わる点に注意。※他人所有ファイルはそもそも処理せずスキップする(救済対象外)。',
    type: 'boolean',
    default: true,
  },
  {
 ...
 */
const FAILURE_CATEGORIES: { code: string; label: string }[] = [
...
function categoryLabel_(code: string): string {
...
/** 「移行元フォルダ src の中身を、移行先フォルダ dst へ移す」作業単位 */
...
interface FailureRecord {
  /** FAILURE_CATEGORIES の code。失敗の種類 */
  category: string;
  fileId: string;
  name: string;
  path: string;
...
interface MigrationStats {
  ...
  filesSkipped: number;
  filesFailed: number;
  /** 失敗の種類 (FAILURE_CATEGORIES の code) ごとの件数 */
  failuresByCategory: { [code: string]: number };
}
...
interface ChildItem {
  ...
  mimeType: string;
  ownedByMe: boolean;
  /** 所有者のメールアドレス (取得できた場合)。スキップ一覧の表示に使う */
  ownerEmail: string;
}

/** 現在の実行で読み込んだ設定のキャッシュ (実行ごとに GAS がリセットする) */
let ACTIVE_CONFIG: Config | null = null;
...
function setupSheets(): void {
  ...
  ss.setActiveSheet(getSheet_(SHEET_SETTINGS) as GoogleAppsScript.Spreadsheet.Sheet);
  const ui = getUiOrNull_();
  if (ui) {
    ui.alert(
      'セットアップ完了',
      '「設定」シートに移行元・移行先のフォルダ ID と各種設定を入力してください。\n\n' +
        '入力できたら メニュー「📁 ドライブ移行」→「② 移行を開始」を実行します。\n' +
        'まずは「ドライラン」にチェックが入った状態で試すことを強く推奨します。',
      ui.ButtonSet.OK
    );
  }
}
...
function startMigration(): void {
  ...
  if (existing && (existing.status === 'RUNNING' || existing.status === 'SUSPENDED')) {
    alertOrLog_(
      '実行できません',
      '進行中の移行ジョブがあります。「③ 進捗を更新して表示」で状況を確認し、' +
  ...
  let rootDstParent: string;
  try {
    const src = withRetry_('移行元フォルダの取得', () =>
      Drive.Files.get(cfg.SOURCE_FOLDER_ID, {
...
function showStatus(): void {
  ...
  if (statusSheet) ss.setActiveSheet(statusSheet);
...
function cancelMigration(): void {
  ...
  alertOrLog_(
    '移行を中止しました',
    `処理済み: ${state.stats.foldersVisited} フォルダ / ${state.stats.filesMoved} ファイル移動。` +
...
function resetState(): void {
  if (
    !confirm_(
      '状態をリセットしますか?',
      '保存された進捗をすべて消去します。移行済みファイルは移行先に残ります。\nこの後「② 移行を開始」で最初からやり直せます。'
    )
  ) {
    return;
  }
  ...
  clearStateStorage_();
  const statusSheet = getSheet_(SHEET_STATUS);
  if (statusSheet) buildStatusSheet_(getSpreadsheet_());
  const failSheet = getSheet_(SHEET_FAILURES);
  if (failSheet) buildFailuresSheet_(getSpreadsheet_());
  const skipSheet = getSheet_(SHEET_SKIPPED);
  if (skipSheet) buildSkippedSheet_(getSpreadsheet_());
...
function trashEmptySourceFolders(): void {
  ...
  if (!state || state.status !== 'DONE') {
    if (
      !confirm_(
        '注意',
        '移行が完了(DONE)していません。空のフォルダしか削除しませんが、続行しますか?'
      )
    ) {
      return;
    }
  } else if (
    !confirm_('空フォルダの片付け', cfg.DRY_RUN ? '（ドライラン）削除予定を確認します。' : '空になった移行元フォルダをゴミ箱へ入れます。')
  ) {
    return;
  }
  ...
  const trashed = trashIfEmptyRecursive_(
    src.id as string,
    src.name || '',
    src.ownedByMe === true
  );
...
function resolveDestination_(destId: string): { rootId: string; label: string } {
  ...
  if (!folder.driveId) {
    throw new Error(
      `移行先 (${folder.name}) は共有ドライブ内にありません。共有ドライブ、または` +
...
function ensureFolder_(
  ...
  const created = withRetry_('移行先フォルダの作成', () =>
    Drive.Files.create({ name: name, mimeType: FOLDER_MIME, parents: [dstParentId] }, null, {
      supportsAllDrives: true,
      fields: 'id',
    })
  );
...
function suspendAndScheduleResume_(state: MigrationState): void {
  ...
  scheduleResume_();
  Logger.log(
    `⏸ 制限時間に達したため中断しました (残りフォルダ: ${state.queue.length})。自動再開します。`
  );
}
...
function finishMigration_(state: MigrationState): void {
  ...
  notify_(
    state.dryRun ? '【DRY_RUN 完了】ドライブ移行の計画確認が終わりました' : '【完了】ドライブ移行が終わりました',
    report
  );
}
...
/** 現在の設定を返す (実行中はキャッシュ)。 */
function cfg_(): Config {
  if (!ACTIVE_CONFIG) ACTIVE_CONFIG = loadConfig_();
 ...
 */
function loadConfig_(): Config {
  const sheet = getSheet_(SHEET_SETTINGS);
  if (!sheet) {
    throw new Error(
      '「設定」シートがありません。メニュー「📁 ドライブ移行」→「① 設定シートを準備 / 初期化」を実行してください。'
    );
  }
  ...
  for (let r = 0; r < values.length; r++) {
    ...
    if (label) byLabel[label] = values[r][COL_VALUE - 1];
  }
  ...
  const result = cfg as unknown as Config;
...
function loadConfigOrAlert_(): Config | null {
  try {
...
/** セル値を設定項目の型に変換する。 */
function coerceValue_(raw: unknown, def: SettingDef): string | number | boolean {
  if (def.type === 'boolean') {
    if (typeof raw === 'boolean') return raw;
    const s = String(raw === null || raw === undefined ? '' : raw)
      .trim()
      .toLowerCase();
    if (s === '') return def.default as boolean;
    return s === 'true' || s === 'はい' || s === 'yes' || s === '1' || s === 'on' || s === '✓';
  }
  if (def.type === 'number') {
    if (raw === '' || raw === null || raw === undefined) return def.default as number;
    const n = Number(raw);
...
function buildSettingsSheet_(ss: GoogleAppsScript.Spreadsheet.Spreadsheet): void {
  let sheet = ss.getSheetByName(SHEET_SETTINGS);
  const existingValues: { [label: string]: unknown } = {};
  if (sheet) {
    const values = sheet.getDataRange().getValues();
    for (let r = 0; r < values.length; r++) {
      const label = String(values[r][COL_LABEL - 1]).trim();
      if (label) existingValues[label] = values[r][COL_VALUE - 1];
    }
    sheet.clear();
  } else {
 ...
 */
function statusRows_(state: MigrationState): Array<[string, string | number]> {
  const s = state.stats;
...
/** 進捗シートを作成/初期化する。 */
function buildStatusSheet_(ss: GoogleAppsScript.Spreadsheet.Spreadsheet): void {
  let sheet = ss.getSheetByName(SHEET_STATUS);
  if (!sheet) sheet = ss.insertSheet(SHEET_STATUS);
  sheet.clear();
...
/** 失敗一覧シートを作成/初期化する。 */
function buildFailuresSheet_(ss: GoogleAppsScript.Spreadsheet.Spreadsheet): void {
  let sheet = ss.getSheetByName(SHEET_FAILURES);
  if (!sheet) sheet = ss.insertSheet(SHEET_FAILURES);
  sheet.clear();
...
function buildSkippedSheet_(ss: GoogleAppsScript.Spreadsheet.Spreadsheet): void {
  let sheet = ss.getSheetByName(SHEET_SKIPPED);
  if (!sheet) sheet = ss.insertSheet(SHEET_SKIPPED);
  sheet.clear();
 ...
 */
function protectSettingsLayout_(sheet: GoogleAppsScript.Spreadsheet.Sheet): void {
  try {
    // 既存の保護を一旦解除
...
function maybeWriteStatus_(state: MigrationState): void {
  const now = Date.now();
...
function writeStatusToSheet_(state: MigrationState): void {
  try {
    const ss = getSpreadsheet_();
    ...
    if (!statusSheet) {
      buildStatusSheet_(ss);
      statusSheet = ss.getSheetByName(SHEET_STATUS) as GoogleAppsScript.Spreadsheet.Sheet;
    }
    ...
    if (!failSheet) {
      buildFailuresSheet_(ss);
      failSheet = ss.getSheetByName(SHEET_FAILURES) as GoogleAppsScript.Spreadsheet.Sheet;
    }
    ...
    if (!skipSheet) {
      buildSkippedSheet_(ss);
      skipSheet = ss.getSheetByName(SHEET_SKIPPED) as GoogleAppsScript.Spreadsheet.Sheet;
    }
...
function saveState_(state: MigrationState): void {
  ...
  if (json.length > STATE_TOTAL_LIMIT) {
    throw new Error(
      `移行状態が大きくなりすぎました (${json.length} 文字)。フォルダ数が非常に多い場合は、` +
...
function getSpreadsheet_(): GoogleAppsScript.Spreadsheet.Spreadsheet {
  ...
  if (!ss) {
    throw new Error(
      'スプレッドシートに紐づいていません。このスクリプトはスプレッドシートの' +
...
function getSheet_(name: string): GoogleAppsScript.Spreadsheet.Sheet | null {
  return getSpreadsheet_().getSheetByName(name);
}

/** UI を返す。トリガー実行など UI が無い文脈では null。 */
function getUiOrNull_(): GoogleAppsScript.Base.Ui | null {
  try {
...
function confirm_(title: string, message: string): boolean {
  const ui = getUiOrNull_();
  if (!ui) return true;
...
function alertOrLog_(title: string, message: string): void {
  const ui = getUiOrNull_();
  if (ui) {
...
function toast_(message: string, title: string, seconds?: number): void {
  try {
...
function withRetry_<T>(label: string, fn: () => T): T {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    if (attempt > 0) {
      const waitMs =
        Math.min(Math.pow(2, attempt - 1) * 1000, 32000) + Math.floor(Math.random() * 500);
    ...
    try {
      ...
      lastError = e;
      if (!isTransientError_(e)) throw e;
...
function recordFailure_(state: MigrationState, failure: FailureRecord): void {
  ...
  if (!state.stats.failuresByCategory) state.stats.failuresByCategory = {};
...
function recordSkip_(state: MigrationState, skip: SkipRecord): void {
