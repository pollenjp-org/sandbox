...
- **スプレッドシート UI**: 設定は「設定」シートのセル(B2 譲渡先 / B3 対象フォルダ / B4 モード / B5 譲渡方式のプルダウン)、実行はカスタムメニュー、結果は「譲渡ログ」シート(台帳)に 1 件 1 行で記録
- **2 つの走査戦略**: 指定フォルダ配下の再帰走査(ツリー走査)と、全所有物の検索走査。対象になるのは常に**実行者本人が所有する**アイテムだけ
- **2 つの譲渡方式**: 直接譲渡(Workspace 同一ドメイン向け・即時)と招待方式(個人アカウント向け・`pendingOwner` による「招待 → 承諾」)。招待方式で移転できるのは Google ネイティブ形式のファイルのみ(フォルダ・非ネイティブ形式は対象外)。受信側はメニュー「所有権の譲渡を承諾する」で受領する
