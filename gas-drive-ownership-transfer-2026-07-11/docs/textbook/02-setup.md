...
├── index.html        # Web アプリ UI の画面(src/ からコピーされる)
...
$ pnpm exec clasp create-script --title "drive-ownership-transfer" --rootDir dist
```
...
```bash
$ mise run open    # 実体: pnpm exec clasp open-script
```
