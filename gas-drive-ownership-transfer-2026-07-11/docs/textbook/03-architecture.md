...
## 3.5 進捗はどこに保存されるのか: スクリプトプロパティ

チェックポイントの保存先は **スクリプトプロパティ**(`PropertiesService.getScriptProperties()`)です。スクリプトに紐づく小さなキー・バリュー保存領域で、実行が終わっても値が残ります(= 永続化)。
...
- **キー・バリューストア**: 「名前(キー)→ 値」の対応だけを保存する単純なデータベース。スクリプトプロパティは値として文字列しか保存できないため、複雑なデータは JSON 文字列に変換して保存します
...
ただしスクリプトプロパティには **「1 つの値につき最大 9KB」** という制限があります。フォルダキューが長くなる(=未処理フォルダ ID が数百件たまる)と 9KB を超える恐れがあるため、本ツールは JSON 文字列を **8,000 文字ずつのチャンク(断片)に分割**して複数のプロパティに保存し、読み出し時に連結して復元します。この実装は第 4 章の `state.ts` で解説します。

## 3.6 二重実行を防ぐ: スクリプトロック

「手動で `startTransfer()` を実行した直後に、前回の再開トリガーも発火してしまった」というように、**同じスクリプトが同時に 2 つ走る**可能性があります。2 つの実行が同じ状態を同時に読み書きすると、進捗が壊れたり同じファイルを二重処理したりします(競合状態)。

これを防ぐのが `LockService` の **スクリプトロック**です。実行の最初にロックを取得し、取れなければ「他の実行が進行中」と判断して身を引きます。トイレの個室の鍵のようなもので、「鍵が閉まっていたら入らない」という仕組みです。
...
| 仕組み | 内容 |
| --- | --- |
| DRY RUN が既定 | `config.ts` の `dryRun` は最初から `true`。書き換えない限り 1 件も譲渡されない |
| 譲渡先の検証 | 空文字・`@` なし・自分自身への譲渡は開始前にエラーで停止 |
| 所有者チェック | 1 件ごとに「本当に自分がオーナーか」を確認してから譲渡(他人のファイルは触らない) |
| 設定のスナップショット | 実行開始時の設定を状態に写し取る。実行途中で `config.ts` を書き換えて push しても、進行中の処理には影響しない |
| エラーでも止まらない | 1 件の譲渡失敗で全体を止めず、記録して続行。失敗件数はサマリで報告 |
| 未完了状態の保護 | 前回の処理が残っている場合、新しい処理は開始できない(明示的に `stopTransfer()` するまで) |
