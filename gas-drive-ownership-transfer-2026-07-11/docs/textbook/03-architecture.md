...
`PropertiesService` には共有範囲の異なる 3 種類の領域があり、本ツールは「スクリプト × 利用者」ごとに独立する**ユーザープロパティ**を採用しています。これは Web アプリ UI(第 7 章)を複数人で使ったとき、**利用者ごとの進捗が互いに見えず・干渉しない**ようにするためです。
...
「手動で `startTransfer()` を実行した直後に、前回の再開トリガーも発火してしまった」というように、**同じ利用者の実行が同時に 2 つ走る**可能性があります。2 つの実行が同じ状態を同時に読み書きすると、進捗が壊れたり同じファイルを二重処理したりします(競合状態)。
