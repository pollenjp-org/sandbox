...
├── mise.toml              # ツールバージョン(Node 22)とタスク定義
...
第 3 章 3.5 で説明した「9KB 制限をチャンク分割で回避する」実装です。保存はこうなっています。
...
譲渡先メールアドレスの妥当性(空でない・`@` を含む・**自分自身でない**)をチェックし、`CONFIG` の値を状態オブジェクトへ写し取ります。以後のバッチは常に状態側の値を使うため、実行途中に `config.ts` を書き換えても進行中の処理はぶれません。
