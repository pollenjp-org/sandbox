...
自由配置の図(ネットワーク構成図、画面遷移のラフなど)を追加したくなった場合に、draw.io のファイル(`.drawio`)をこのディレクトリへ置きます。

現時点で本プロジェクトの図はすべて UML 系のため、テキストで差分管理できる [PlantUML](../plantuml/) に統一しており、このディレクトリは手引きのみです。
...
- ドキュメントへ埋め込む場合は SVG でエクスポートし、`docs/textbook/images/` へ置く
